...
(defonce ^:private ^clojure.lang.AtomicLong id-gen (clojure.lang.AtomicLong.))   ;;; ^java.util.concurrent.atomic.AtomicLong
  ...
  (let [rand (ThreadLocalRandom/current)
    ...
    (loop [i 1]
      (if (= i n)
        a
        (do
          (let [j (.Next rand (int (inc i)))]                                     ;;; (.nextInt rand (inc i))
            (aset a i (aget a j))
            (aset a j i)
            (recur (inc i))))))))
